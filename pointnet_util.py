...
def farthest_point_sample(xyz, npoint):
    ...
    for i in range(npoint):
        ...
        dist = torch.tensor(torch.sum((xyz - centroid) ** 2, -1), dtype=torch.float32)
        mask = dist < distance
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    mask = group_idx == N
    # print(group_idx.size(), group_first.size(), mask.size())
    if group_first.size() != group_idx.size():
        print("EOP")
...
class PointNetSetAbstractionMsg(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            grouped_points = torch.tensor(grouped_points.permute(0, 3, 2, 1), dtype=torch.float32) # [B, D, K, S]
